...
export default function DashboardPage() {
  ...
  // FIX: antes, una vez generada la rutina de hoy, el Dashboard siempre volvía
  // a mostrar esa misma rutina (incluso ya completada al 100%) sin dar opción
  // de elegir intensidad para una sesión nueva. Ahora, si todos los ejercicios
  // de la rutina cargada ya están marcados como completados (dato que llega
  // del backend), se ofrece el selector de volumen/intensidad de nuevo.
  const [nuevaSesion, setNuevaSesion] = useState(false);

  const rutinaCompletada =
    rutina?.ejercicios?.length > 0 && rutina.ejercicios.every((e) => e.completado);
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white font-sans p-6 selection:bg-yellow-400 selection:text-neutral-900">
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        {/* Rutina de hoy ya completada al 100% → ofrecer sesión nueva */}
        {rutina && rutinaCompletada && !nuevaSesion && (
          <div className="rounded-none bg-[#111111] border-2 border-green-500 p-8 text-center space-y-4">
            <span role="img" aria-label="Completado" className="text-3xl block">✅</span>
            ...
            <p className="text-neutral-400 text-xs max-w-sm mx-auto uppercase tracking-wide leading-relaxed">
              Todos los ejercicios de hoy están registrados. Si quieres entrenar de nuevo,
              elige la intensidad para una nueva sesión.
            </p>
            <button
        ...
        {/* Rutina cargada (pendiente o en curso) */}
