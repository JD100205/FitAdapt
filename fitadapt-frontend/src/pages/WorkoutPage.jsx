...
// FIX: la rutina de hoy ahora trae, por ejercicio, un flag "completado" calculado
// en el backend a partir del Historial del día. Esto evita dos problemas:
// 1) Si volvías a "Entrenar" después de terminar la sesión, antes se mostraba
//    todo como pendiente y se podía volver a registrar el mismo ejercicio
//    (sumando puntos otra vez).
// 2) El Dashboard ahora puede saber si la sesión de hoy ya se completó por
//    completo, para ofrecer una sesión nueva con selector de intensidad.
function obtenerCompletadosIds(rutina) {
  return rutina?.ejercicios?.filter((e) => e.completado).map((e) => e.idEjercicio) ?? [];
}
...
function ExerciseCard({ ejercicio, onCompletado }) {
  ...
  const [completado, setCompletado] = useState(Boolean(ejercicio.completado));
  ...
  const handleCompletar = async () => {
    if (completado || cargando || !cronometroIniciado) return;
    setCargando(true);
    try {
      const minutosReales = Math.max(Math.round(segundosTranscurridos / 60), 1);
        ...
        {!completado && (
          <div className="mt-4 pt-4 border-t border-neutral-800/60 font-mono">
            <Timer
              ...
              onIniciar={() => setCronometroIniciado(true)}
            />
          </div>
        )}
        ...
        {!completado && (
          <>
            <button
              ...
              disabled={cargando || !cronometroIniciado}
              className="mt-5 w-full bg-yellow-400 hover:bg-yellow-500 disabled:opacity-50 disabled:cursor-not-allowed text-neutral-900 font-black text-xs py-3 px-4 rounded-none uppercase tracking-widest"
            >
              {cargando ? 'Registrando…' : 'Finalizar Bloque'}
            </button>
            {!cronometroIniciado && (
              <p className="mt-2 text-[10px] text-neutral-500 uppercase tracking-wider text-center">
                Inicia el cronómetro para poder completar el ejercicio
              </p>
            )}
          </>
        )}
...
export default function WorkoutPage() {
  ...
  const [completados, setCompletados] = useState(() => obtenerCompletadosIds(location.state?.rutina));
    ...
    if (!rutina && user?.idUsuario) {
      getRutinaHoy(user.idUsuario)
        .then((res) => {
          if (res.data) {
            setRutina(res.data);
            setCompletados(obtenerCompletadosIds(res.data));
          } else {
